...
from dartfx.ddi.ddicdi.assistants import CdiClassAssistant, CdiAssistant
...
def benchmark_instantiation(n=100000):
    ...
    start = time.time()
    for _ in range(n):
        wrapper = CdiAssistant(resource=res)
    end = time.time()
...
def benchmark_full_creation(n=10000):
    ...
    start = time.time()
    for _ in range(n):
        assistant = CdiClassAssistant.create(model.InstanceVariable)
    end = time.time()
