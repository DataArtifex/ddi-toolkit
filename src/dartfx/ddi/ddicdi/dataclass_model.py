            ...
            if attribute_info.is_list:
                ...
                if ddi:  # ddiIdentifier: a InternationalRegistrationDataIdentifier
                    if self.identifier.ddiIdentifier is None:
                        self.identifier.ddiIdentifier = InternationalRegistrationDataIdentifier(
                ...
                if nonddi:  # nonDdiIdentifier: NonDdiIdentifier
                    if self.identifier.nonDdiIdentifier is None:
                        self.identifier.nonDdiIdentifier = []
                    # add to the list
...
class Agent:
    ...
    purpose: "InternationalString" = field(
        default=None
...
class AuthorizationSource(DdiCdiClass):
    ...
    legalMandate: "InternationalString" = field(
        default=None
    )  # Provide a legal citation to a law authorizing the study/data collection.
    purpose: "InternationalString" = field(
        default=None
    )  # Intent or reason for the object/the description of the object.
    statementOfAuthorization: "InternationalString" = field(
        default=None
    )  # Text of the authorization (law, mandate, approved business case).
    AuthorizationSource_has_Agent: list["Agent"] = field(
        default_factory=list, metadata={"association": "Agent"}
    )  # Agent responsible for the authorization source.
...
class CatalogDetails(DdiCdiClass):
    ...
    access: list["AccessInformation"] | None = None  # Information important for understanding access conditions.
    alternativeTitle: list["InternationalString"] | None = (
        None  # An alternative title by which a data collection is commonly referred, or an abbreviation for the title.
    )
    contributor: list["AgentInRole"] | None = (
        None  # The name of a contributing author or creator, who worked in support of the primary creator given above.
    )
    creator: list["AgentInRole"] | None = (
        None  # Person, corporate body, or agency responsible for the substantive and intellectual content of the described object.
    )
    date: list["CombinedDate"] | None = (
        None  # A date associated with the annotated object (not the coverage period). Use typeOfDate to specify the type of date such as Version, Publication, Submitted, Copyrighted, Accepted, etc.
    )
    identifier: Optional["InternationalIdentifier"] = (
        None  # An identifier or locator. Contains identifier and Managing agency (ISBN, ISSN, DOI, local archive). Indicates if it is a URI.
    )
    informationSource: list["InternationalString"] | None = (
        None  # The name or identifier of source information for the annotated object.
    )
    languageOfObject: list[str] | None = (
    ...
    publisher: list["AgentInRole"] | None = (
        None  # Person or organization responsible for making the resource available in its present form.
    )
    relatedResource: list["Reference"] | None = (
    ...
    subTitle: list["InternationalString"] | None = None  # Secondary or explanatory title.
    summary: Optional["InternationalString"] = None  # A summary description (abstract) of the annotated object.
    title: Optional["InternationalString"] = (
        None  # Full authoritative title. List any additional titles for this item as alternativeTitle.
    )
    typeOfResource: list["ControlledVocabularyEntry"] | None = (
        None  # Provide the type of the resource. This supports the use of a controlled vocabulary. It should be appropriate to the level of the annotation.
    )
...
class ComponentPosition(DdiCdiClass):
    ...
    identifier: "Identifier" = (
        None  # Identifier for objects requiring short- or long-lasting referencing and management.
    )
    value: int = None  # Index value of the member in an ordered array.
    indexesDataStructureComponent: "DataStructureComponent" = field(
        metadata={"association": "DataStructureComponent"}, default=None
    )  #
...
class Concept(DdiCdiClass):
    ...
    catalogDetails: Optional["CatalogDetails"] = (
        None  # Bundles the information useful for a data catalog entry. Examples would be creator, contributor, title, copyright, embargo, and license information. A set of information useful for attribution, data discovery, and access. This is information that is tied to the identity of the object. If this information changes the version of the associated object changes.
    )
    definition: Optional["InternationalString"] = (
        None  # Natural language statement conveying the meaning of a concept, differentiating it from other concepts. Supports the use of multiple languages and structured text. 'externalDefinition' can't be used if 'definition' is used.
    )
    displayLabel: list["LabelForDisplay"] | None = (
        None  # A human-readable display label for the object. Supports the use of multiple languages. Repeat for labels with different content, for example, labels with differing length limitations.
    )
    externalDefinition: Optional["Reference"] = (
    ...
    name: list["ObjectName"] | None = None  # Human understandable name (linguistic signifier, word)
...
class ConceptSystem(DdiCdiClass):
    ...
    name: list["ObjectName"] | None = (
        None  # Human understandable name (linguistic signifier, word, phrase, or mnemonic). May follow ISO/IEC 11179-5 naming principles, and have context provided to specify usage.
    )
    purpose: Optional["InternationalString"] = None  # Intent or reason for the object/the description of the object.
    isDefinedBy_Concept: list["Concept"] | None = (
        None  # Concept system is defined by zero to many concepts. The conceptual basis for the collection of members.
    )
    has_Concept: list["Concept"] | None = None  # Concept system has zero to many concepts.
...
class CategorySet(ConceptSystem):
    ...
    hasCategoryPosition: list["CategoryPosition"] = field(
        default_factory=list, metadata={"association": "CategoryPosition"}
    )
...
class Category(Concept):
    ...
    descriptiveText: "InternationalString" = field(
        default=None
...
class ClassificationItem(DdiCdiClass):
    ...
    changeFromPreviousVersion: Optional["InternationalString"] = field(
        default=None
    ...
    futureNotes: list["InternationalString"] = field(
        default_factory=list
    )  # The future events describe an intended or implemented change (or a number of changes) related to an invalid item.
    identifier: Optional["Identifier"] = field(
        default=None
    )  # Identifier for objects requiring short- or long-lasting referencing and management.
    ...
    isValid: bool | None = field(default=None)  # Indicates whether or not the item is currently valid.
    name: list["ObjectName"] = field(
        default_factory=list
    )  # Human understandable name (linguistic signifier, word, phrase, or mnemonic).
    validDates: Optional["DateRange"] = field(default=None)  # The dates describing the validity period of the object.
    ClassificationItem_excludes_ClassificationItem: list["ClassificationItem"] = field(
        default_factory=list, metadata={"association": "ClassificationItem"}
    )
    ClassificationItem_hasRulingBy_AuthorizationSource: list["AuthorizationSource"] = field(
        default_factory=list, metadata={"association": "AuthorizationSource"}
    )
...
class ConceptualDomain(DdiCdiClass):
    ...
    displayLabel: list["LabelForDisplay"] = field(
        default_factory=list
    )  # A human-readable display label for the object. Supports the use of multiple languages. Repeat for labels with different content, for example, labels with differing length limitations.
    identifier: "Identifier" = (
        None  # Identifier for objects requiring short- or long-lasting referencing and management.
    )
    isDescribedBy_ValueAndConceptDescription: "ValueAndConceptDescription" = field(
        default=None, metadata={"association": "ValueAndConceptDescription"}
    ...
    takesConceptsFrom_ConceptSystem: "ConceptSystem" = field(
        default=None, metadata={"association": "ConceptSystem"}
...
class ConceptualValue(Concept):
    ...
    hasConceptFrom_ConceptualDomain: "ConceptualDomain" = (
        None  # Conceptual value has concept from one conceptual domain.
    )
...
class ConceptualVariable(Concept):
    ...
    descriptiveText: "InternationalString" = (
        None  # A short natural language account of the characteristics of the object.
    )
    ...
    measures: list["UnitType"] = field(
        default=None, metadata={"association": "UnitType"}
    )  # The measures association is intended to describe specific relationships between the ConceptualVariable and UnitType classes, and similar relationships between their sub-classes.
    takesSentinelConceptsFrom: list["SentinelConceptualDomain"] = field(
        default=None, metadata={"association": "SentinelConceptualDomain"}
    )  # Identifies the conceptual domain containing the set of sentinel concepts used to describe the conceptual variable.
    takesSubstantiveConceptsFrom: list["SubstantiveConceptualDomain"] = field(
        default=None, metadata={"association": "SubstantiveConceptualDomain"}
...
class DataPoint(DdiCdiClass):
    ...
    correspondsTo_DataStructureComponent: list["DataStructureComponent"] = field(
        default_factory=list, metadata={"association": "DataStructureComponent"}
    )  #
...
class DataStore(DdiCdiClass):
    ...
    isDefinedBy_Concept: list["Concept"] = field(
        default=None, metadata={"association": "Concept"}
    )  # The conceptual basis for the collection of members.
    has_LogicalRecordPosition: list["LogicalRecordPosition"] = field(
        default=None, metadata={"association": "LogicalRecordPosition"}
    )
    has_LogicalRecord: list["LogicalRecord"] = field(default=None, metadata={"association": "LogicalRecord"})
    has_RecordRelation: "RecordRelation" = field(
        default=None, metadata={"association": "RecordRelation"}
...
class DataStructureComponent(DdiCdiClass):
    ...
    semantic: list["PairedControlledVocabularyEntry"] = field(
        default_factory=list
    )  # Qualifies the purpose or use expressed as a paired external controlled vocabulary.
    specialization: "SpecializationRole" = field(
        default=None
    ...
    isDefinedBy_RepresentedVariable: "RepresentedVariable" = field(
        default=None
...
class DataStructure(DataStructureComponent):
    ...
    def add_represented_variable(
        self, represented_variable: "RepresentedVariable", position_value: None
...
class DimensionalDataStructure(DataStructure):
    ...
    DimensionalDataStructure_uses_DimensionGroup: list["DimensionGroup"] = field(
        default_factory=list, metadata={"association": "DimensionGroup"}
    )
...
class EnumerationDomain(DdiCdiClass):
    ...
    usesLevelStructure: "LevelStructure" = field(
        default=None, metadata={"association": "LevelStructure"}
    )  # Has meaningful level to which members belong.
    referencesCategorySet: "CategorySet" = field(
        default=None, metadata={"association": "CategorySet"}
    )  # Category set associated with the enumeration.
    isDefinedByConcept: list["Concept"] = field(
        default_factory=list, metadata={"association": "Concept"}
    )  # The conceptual basis for the collection of members.
...
class CodeList(EnumerationDomain):
    ...
    allowsDuplicates: bool = field(
        default=False
    )  # If value is False, the members are unique within the collection - if True, there may be duplicates.
...
class Code(DdiCdiClass):
    ...
    denotesCategory: "Category" = field(
        default=None, metadata={"association": "Category"}
...
class InstanceValue(DdiCdiClass):
    ...
    whiteSpace: Optional["WhiteSpaceRule"] = (
    ...
    hasValueFrom_ValueDomain: Optional["ValueDomain"] = field(
        default=None,
        metadata={
            "association": "ValueDomain, DescriptorValueDomain, ReferenceValueDomain, SentinelValueDomain, SubstantiveValueDomain"
        },
    )
    isStoredIn_DataPoint: Optional["DataPoint"] = field(default=None, metadata={"association": "DataPoint"})
    represents_ConceptualValue: Optional["ConceptualValue"] = field(
        default=None, metadata={"association": "ConceptualValue"}
    )
...
class RepresentedVariable(ConceptualVariable):
    ...
    describedUnitOfMeasure: "ControlledVocabularyEntry" = field(
        default=None
    )  # The unit in which the data values are measured (kg, pound, euro), expressed as a value from a controlled system of entries (i.e., QDT).
    hasIntendedDataType: "ControlledVocabularyEntry" = field(
        default=None
    )  # The data type intended to be used by this variable.
    simpleUnitOfMeasure: str = field(
        default=None
    )  # The unit in which the data values are measured (kg, pound, euro), expressed as a simple string.
    takesSentinelValues: list["SentinelValueDomain"] = field(
        default=None, metadata={"association": "SentinelValueDomain"}
    )  # A represented variable may have more than one sets of sentinel value domains, one for each type of software platform on which related instance variables might be instantiated.
    takesSubstantiveValues: list["SubstantiveValueDomain"] = field(
        default=None, metadata={"association": "SubstantiveValueDomain"}
    )  # The substantive representation (substantive value domain) of the variable.
...
class InstanceVariable(RepresentedVariable):
    ...
    platformType: "ControlledVocabularyEntry" = field(
        default=None
    )  # Describes the application or technical system context in which the variable has been realized.
    source: "Reference" = field(default=None)  # Reference capturing provenance information.
    variableFunction: list["ControlledVocabularyEntry"] = field(
        default=None
    )  # Immutable characteristic of the variable.
    hasPhysicalSegmentLayout: list["PhysicalSegmentLayout"] = field(
        default=None, metadata={"association": "PhysicalSegmentLayout"}
    )  #
...
class KeyDefinition(DdiCdiClass):
    ...
    correspondsTo_Universe: "Universe" = None  #
    correspondsTo_Unit: "Unit" = None  #
    has_KeyDefinitionMember: list["KeyDefinitionMember"] = field(
        default_factory=list, metadata={"association": "KeyDefinitionMember"}
    )  #
...
class KeyMember(InstanceValue):
    ...
    isBasedOn_DataStructureComponent: list["DataStructureComponent"] = field(
        default=None, metadata={"association": "DataStructureComponent"}
    )
...
class Level:
    ...
    Level_isDefinedBy_Concept: Optional["Concept"] = field(
        default=None, metadata={"association": "Concept"}
    )  # A concept or concept sub-type which describes the level.
    Level_groups_ClassificationItem: list["ClassificationItem"] = field(
        default_factory=list, metadata={"association": "ClassificationItem"}
    )  # Classification items belonging to this level.
...
class LogicalRecord(DdiCdiClass):
    ...
    LogicalRecord_isDefinedBy_Concept: list["Concept"] = field(
        default=None, metadata={"association": "Concept"}
    )  # The conceptual basis for the collection of members.
    LogicalRecord_has_InstanceVariable: list["InstanceVariable"] = field(
        default=None, metadata={"association": "InstanceVariable"}
    )
...
class Notation(DdiCdiClass):
    ...
    whiteSpace: "WhiteSpaceRule" = field(
        default=None
    )  # The usual setting "collapse" states that leading and trailing white space will be removed and multiple adjacent white spaces will be treated as a single white space. When setting to "replace" all occurrences of #x9 (tab), #xA (line feed) and #xD (carriage return) are replaced with #x20 (space) but leading and trailing spaces will be retained. If the existence of any of these white spaces is critical to the understanding of the content, change the value of this attribute to "preserve".
    representsCategory: list["Category"] = field(
        default_factory=list, metadata={"association": "Category"}
...
class PhysicalDataSet(DdiCdiClass):
    ...
    name: Optional["ObjectName"] = (
        None  # Human understandable name (linguistic signifier, word, phrase, or mnemonic). May follow ISO/IEC 11179-5 naming principles, and have context provided to specify usage.
    )
    numberOfSegments: int | None = (
        None  # The number of distinct segments (e.g., segments patterns with different structures, identified separately) in a physical dataset.
    )
    overview: Optional["InternationalString"] = (
        None  # Short natural language account of the information obtained from the combination of properties and relationships associated with an object.
    )
    physicalFileName: str | None = None  # Use when multiple physical segments are stored in a single file.
    purpose: Optional["InternationalString"] = None  # Intent or reason for the object/the description of the object.
    ...
    formatsDataStore: list["DataStore"] = field(
        default_factory=list
...
class PhysicalRecordSegment(DdiCdiClass):
    ...
    physicalFileName: str | None = None  # Use when each physical segment is stored in its own file.
    purpose: Optional["InternationalString"] = None  # Intent or reason for the object/the description of the object.
    representsPopulation: Optional["Population"] = (
        None  # A record segment may represent a specific population or sub-population within a larger set of segments. Allows for the identification of this filter for membership in the segment.
    )
    isDefinedBy: list["Concept"] = field(default_factory=list)  # The conceptual basis for the collection of members.
    hasPhysicalSegmentLayout: Optional["PhysicalSegmentLayout"] = None
    mapsToLogicalRecord: Optional["LogicalRecord"] = (
        None  # Every data record has zero to many physical record segments. It is possible to describe a physical data product and its record segment(s) without reference to a data record.
    )
...
class PhysicalRecordSegmentPosition(DdiCdiClass):
    ...
    indexes_PhysicalRecordSegment: "PhysicalRecordSegment" = field(
        default=None, metadata={"association": "PhysicalRecordSegment"}
...
class PhysicalSegmentLayout(DdiCdiClass):
    ...
    allowsDuplicates: bool = True  # If value is False, the members are unique within the collection - if True, there may be duplicates. (Note that a mathematical “bag” permits duplicates and is unordered - a “set” does not have duplicates and may be ordered.)
    arrayBase: int | None = (
        None  # The starting value for the numbering of cells, rows, columns, etc. when they constitute an ordered sequence (an array). Note that in DDI, this is typically either 0 or 1.
    )
    catalogDetails: Optional["CatalogDetails"] = (
        None  # Bundles the information useful for a data catalog entry. Examples would be creator, contributor, title, copyright, embargo, and license information. A set of information useful for attribution, data discovery, and access. This is information that is tied to the identity of the object. If this information changes the version of the associated object changes.
    )
    commentPrefix: str | None = (
        None  # A string used to indicate that an input line is a comment, a string which precedes a comment in the data file.
    )
    delimiter: str | None = None  # The Delimiting character in the data. Must be used if isDelimited is True.
    encoding: Optional["ControlledVocabularyEntry"] = None  # The character encoding of the represented data.
    escapeCharacter: str | None = (
        None  # The string that is used to escape the quote character within escaped cells, or null.
    )
    hasHeader: bool | None = None  # True if the file contains a header containing column names.
    headerIsCaseSensitive: bool | None = None  # If True, the case of the labels in the header is significant.
    headerRowCount: int | None = None  # The number of lines in the header.
    identifier: Optional["Identifier"] = (
        None  # Identifier for objects requiring short- or long-lasting referencing and management.
    )
    ...
    nullSequence: str | None = None  # A string indicating a null value.
    overview: Optional["InternationalString"] = (
        None  # Short natural language account of the information obtained from the combination of properties and relationships associated with an object.
    )
    purpose: Optional["InternationalString"] = None  # Intent or reason for the object/the description of the object.
    quoteCharacter: str | None = None  # The string that is used around escaped cells, or null.
    skipBlankRows: bool | None = None  # If the value is True, blank rows are ignored.
    skipDataColumns: int | None = None  # The number of columns to skip at the beginning of the row.
    skipInitialSpace: bool | None = (
        None  # If the value is True, skip whitespace at the beginning of a line or following a delimiter.
    )
    skipRows: int | None = None  # Number of input rows to skip preceding the header or data.
    tableDirection: Optional["TableDirectionValues"] = (
        None  # Indicates the direction in which columns are arranged in each row.
    )
    textDirection: Optional["TextDirectionValues"] = None  # Indicates the reading order of text within cells.
    treatConsecutiveDelimitersAsOne: bool | None = (
        None  # If the value is True, consecutive (adjacent) delimiters are treated as a single delimiter.
    )
    trim: Optional["TrimValues"] = (
        None  # Specifies which spaces to remove from a data value (start, end, both, neither).
    )
    isDefinedBy: list["Concept"] = field(default_factory=list)  # The conceptual basis for the collection of members.
    formatsLogicalRecord: Optional["LogicalRecord"] = (
        None  # Logical record physically represented by the physical layout.
    )
...
class Universe(UnitType):
    ...
    isInclusive: bool = field(
        default=True
    )  # Default value is True. The description statement of a universe is generally stated in inclusive terms.
...
class PrimaryKey(DdiCdiClass):
    ...
    isComposedOf_PrimaryKeyComponent: list["PrimaryKeyComponent"] = field(
        default_factory=list, metadata={"association": "PrimaryKeyComponent"}
    )  #
...
class PrimaryKeyComponent(DdiCdiClass):
    ...
    correspondsTo_DataStructureComponent: "DataStructureComponent" = field(
        metadata={"association": "DataStructureComponent"}
    )  #
    identifier: "Identifier" = (
        None  # Identifier for objects requiring short- or long-lasting referencing and management.
    )
...
class Unit(DdiCdiClass):
    ...
    definition: "InternationalString" = (
        None  # Natural language statement conveying the meaning of a concept, differentiating it from other concepts.
    )
    displayLabel: list["LabelForDisplay"] = field(
        default_factory=list
    ...
    has_UnitType: list["UnitType"] = field(
        default_factory=list, metadata={"association": "UnitType"}
...
class ValueDomain(DdiCdiClass):
    ...
    recommendedDataType: list["ControlledVocabularyEntry"] = field(
        default_factory=list
...
class SentinelValueDomain(ValueDomain):
    ...
    platformType: "ControlledVocabularyEntry" = field(
        default=None
    )  # The type of platform under which sentinel codes will be used.
    takesConceptsFrom: "SentinelConceptualDomain" = field(
        default=None, metadata={"association": "SentinelConceptualDomain"}
    )  # Corresponding conceptual definition given by a sentinel conceptual domain.
    takesValuesFrom: "EnumerationDomain" = field(
        default=None, metadata={"association": "CodeList,EnumerationDomain,StatisticalClassification"}
    )  # Any subtype of an enumeration domain enumerating the set of valid values.
    isDescribedBy: "ValueAndConceptDescription" = field(
        default=None, metadata={"association": "ValueAndConceptDescription"}
...
class SubstantiveValueDomain(ValueDomain):
    ...
    takesConceptsFrom: "SubstantiveConceptualDomain" = field(
        default=None, metadata={"association": "SubstantiveConceptualDomain"}
...
class ValueMapping:
    ...
    decimalPositions: int | None = field(
        default=None
    )  # The number of decimal positions expressed as an integer. Used when the decimal position is implied (no decimal separator is present)
    defaultDecimalSeparator: str | None = field(
        default=None
    )  # Default value is "." (period). The character separating the integer part from the fractional part of a decimal or real number.
    defaultDigitGroupSeparator: str | None = field(
        default=None
    )  # Default value is null. A character separating groups of digits (for readability).
    format: Optional["ControlledVocabularyEntry"] = field(
        default=None
    )  # This defines the format of the physical representation of the value.
    identifier: Optional["Identifier"] = field(
        default=None
    )  # Identifier for objects requiring short- or long-lasting referencing and management.
    isRequired: bool | None = field(
        default=None
    )  # If the value of this property is True indicates that a value is required for the referenced instance variable.
    length: int | None = field(default=None)  # The length in characters of the physical representation of the value.
    maximumLength: int | None = field(
        default=None
    )  # The largest possible value of the length of the physical representation of the value.
    minimumLength: int | None = field(
        default=None
    )  # The smallest possible value for the length of the physical representation of the value.
    nullSequence: str | None = field(default=None)  # A string indicating a null value.
    numberPattern: str | None = field(default=None)  # A pattern description of the format of a numeric value.
...
class VariableStructure(DdiCdiClass):
    ...
    semantics: "ControlledVocabularyEntry" = (
        None  # Specifies the semantics of the object in reference to a vocabulary, ontology, etc.
    )
    ...
    totality: "StructureExtent" = (
        None  # Indicates whether the related collections are comprehensive in terms of their coverage.
    )
    VariableStructure_structures_VariableCollection: "VariableCollection" = field(
        default=None, metadata={"association": "VariableCollection"}
    )  # Variable structure structures zero to one variable collection.
    VariableStructure_has_VariableRelationship: list["VariableRelationship"] = field(
        default=None, metadata={"association": "VariableRelationship"}
    )  #
...
class Address(DdiCdiDataType):
    ...
    cityPlaceLocal: str | None = None  # City, place, or local area used as part of an address.
    ...
    isPreferred: bool | None = (
        None  # Set to True if this is the preferred location for contacting the organization or individual.
    )
    line: list[str] | None = None  # Number and street including office or suite number. May use multiple lines.
    locationName: Optional["ObjectName"] = None  # Name of the location if applicable.
    postalCode: str | None = None  # Postal or ZIP Code.
    privacy: Optional["ControlledVocabularyEntry"] = (
        None  # Specify the level privacy for the address as public, restricted, or private. Supports the use of an external controlled vocabulary.
    )
    ...
    stateProvince: str | None = (
        None  # A major sub-national division such as a state or province used to identify a major region within an address.
    )
    timeZone: Optional["ControlledVocabularyEntry"] = None  # Time zone of the location expressed as code.
    typeOfAddress: Optional["ControlledVocabularyEntry"] = (
        None  # Indicates address type (i.e. home, office, mailing, etc.).
    )
    typeOfLocation: Optional["ControlledVocabularyEntry"] = (
        None  # The type or purpose of the location (i.e. regional office, distribution center, home).
    )
...
class AccessInformation(DdiCdiDataType):
    ...
    copyright: list["InternationalString"] | None = None  # The copyright statement.
    embargo: list["EmbargoInformation"] | None = None  # Specific information about any relevant embargo.
    license: list["LicenseInformation"] | None = None  # Information about any relevant license.
    rights: list["InternationalString"] | None = (
        None  # Information about rights held in and over the resource. Typically, rights information includes a statement about various property rights associated with the resource, including intellectual property rights.
    )
...
class AgentInRole(DdiCdiDataType):
    ...
    agentName: Optional["BibliographicName"] = (
        None  # Full name of the contributor. Language equivalents should be expressed within the international string structure.
    )
    reference: Optional["Reference"] = None  # Reference to an agent described in DDI or some other platform.
    role: list["PairedControlledVocabularyEntry"] | None = (
        None  # Role of the of the agent within the context of the parent property name.
    )
...
class InternationalString(DdiCdiDataType):
    ...
    languageSpecificString: list["LanguageString"] | None = (
        None  # A non-formatted string of text with an attribute that designates the language of the text. Repeat this object to express the same content in another language.
    )
...
class BibliographicName(InternationalString):
    ...
    affiliation: str | None = (
        None  # The affiliation of this person to an organization. This is generally an organization or sub-organization name and should be related to the specific role within which the individual is being listed.
    )
...
class CombinedDate(DdiCdiDataType):
    ...
    nonIsoDate: list["NonIsoDate"] | None = (
        None  # A simple date expressed in a non-ISO date format, including a specification of the date format and calendar used.
    )
    semantics: Optional["ControlledVocabularyEntry"] = (
        None  # Use to specify the type of date. This may reflect the refinements of dc:date such as dateAccepted, dateCopyrighted, dateSubmitted, etc.
    )
...
class ContactInformation(DdiCdiDataType):
    ...
    address: list["Address"] | None = None  # The address for contact.
    email: list["Email"] | None = None  # Email contact information.
    emessaging: list["ElectronicMessageSystem"] | None = None  # Electronic messaging other than email.
    telephone: list["Telephone"] | None = None  # Telephone for contact.
    website: list["WebLink"] | None = None  # The URL of the Agent's website.
...
class ControlledVocabularyEntry(DdiCdiDataType):
    ...
    entryReference: list["Reference"] | None = (
        None  # A reference to the specific item in the vocabulary referenced in the vocabulary attribute, using a URI or other resolvable identifier.
    )
    entryValue: list[str] | None = (
        None  # The value of the entry of the controlled vocabulary. If no controlled vocabulary is used the term is entered here and none of the properties defining the controlled vocabulary location are used.
    )
    name: str | None = None  # The name of the code list (controlled vocabulary).
    valueForOther: str | None = (
        None  # If the value of the string is "Other" or the equivalent from the codelist, this attribute can provide a more specific value not found in the codelist.
    )
    vocabulary: Optional["Reference"] = (
        None  # A reference to the external controlled vocabulary, using a URI or other resolvable identifier.
    )
...
class ElectronicMessageSystem(DdiCdiDataType):
    ...
    typeOfService: "ControlledVocabularyEntry" = (
        None  # Indicates the type of service used. Supports the use of a controlled vocabulary.
    )
...
class EmbargoInformation(DdiCdiDataType):
    ...
    period: list["DateRange"] | None = None  # The time range(s) for embargo of the object.
...
class FundingInformation(DdiCdiDataType):
    ...
    fundingAgent: list["AgentInRole"] = (
        None  # A reference to the agent (e.g. organization) that provided funding for a grant.
    )
...
class GeoRole(SpecializationRole):
    ...
    geography: Optional["ControlledVocabularyEntry"] = (
        None  # Function in relation to the specification of a place or physical area or feature, ideally drawn from a controlled vocabulary.
    )
...
class Identifier(DdiCdiDataType):
    ...
    ddiIdentifier: Optional["InternationalRegistrationDataIdentifier"] = (
        None  # A globally unique identifier. The values of the three attributes can be used to create a DDI URN.
    )
    isDdiIdentifierPersistent: bool | None = (
        None  # Default value is False indicating that the content of the current version may change (may be in development mode). Set to True when the content of this version will no longer change.
    )
    isDdiIdentifierUniversallyUnique: bool | None = (
        None  # Default value is False. If the id of the object was created as a Universally Unique ID (UUID) set to True.
    )
    nonDdiIdentifier: list["NonDdiIdentifier"] | None = field(
        default_factory=list
    )  # Any identifier other than a DDI identifier.
    uri: Optional["XsdAnyUri"] = (
        None  # A Universal Resource Identifier, valid according to the W3C XML Schema specification.
    )
    ...
    versionResponsibility: Optional["AgentInRole"] = (
        None  # Contributor who has the ownership and responsibility for the current version.
    )
...
class InternationalIdentifier(DdiCdiDataType):
    ...
    managingAgency: "ControlledVocabularyEntry" = (
        None  # The identification of the Agency which assigns and manages the identifier, i.e., ISBN, ISSN, DOI, etc.
    )
...
class LabelForDisplay(InternationalString):
    ...
    validDates: "DateRange" = (
        None  # Allows for the specification of a starting date and ending date for the period that this label is valid.
    )
...
class LanguageString(DdiCdiDataType):
    ...
    isTranslatable: bool | None = (
        None  # Indicates whether content is translatable (True) or not (False). An example of something that is not translatable would be a MNEMONIC of an object or a number.
    )
    isTranslated: bool | None = None  # Indicates whether content is a translation (True) or an original (False).
    language: str | None = None  # Indicates the natural language of content.
    scope: str | None = (
        None  # Supports specification of scope for the contained content. Use with the language specification to filter application of content.
    )
    structureUsed: Optional["ControlledVocabularyEntry"] = (
        None  # The structure type used. Examples are HTML or restructured text.
    )
    translationDate: Optional["XsdDate"] = (
        None  # The date the content was translated. Provision of translation date allows user to verify if translation was available during data collection or other time linked activity.
    )
    translationSourceLanguage: list[str] | None = (
        None  # Lists the natural language(s) of the source. Repeat if source is in multiple languages.
    )
...
class LicenseInformation(DdiCdiDataType):
    ...
    contact: list["ContactInformation"] | None = None  # Information on whom to contact for details on licensing.
    description: list["InternationalString"] | None = None  # A description of licensing terms.
    licenseAgent: list["AgentInRole"] | None = (
        None  # Points to a description of an agent with information about, or responsible for licensing of the object.
    )
    licenseReference: list["Reference"] | None = (
        None  # Points to published license terms, such as to a specific Creative Commons license.
    )
...
class NonDdiIdentifier(DdiCdiDataType):
    ...
    version: str | None = (
        None  # The version of the object being identified, according to the versioning system provided by the identified scheme.
    )
    managingAgency: str | None = None  # The authority which maintains the identification scheme.
...
class NonIsoDate(DdiCdiDataType):
    ...
    calendar: Optional["ControlledVocabularyEntry"] = (
        None  # Specifies the type of calendar used (e.g., Gregorian, Julian, Jewish).
    )
    nonIsoDateFormat: Optional["ControlledVocabularyEntry"] = (
        None  # Indicate the structure of the date provided in NonISODate. For example if the NonISODate contained 4/1/2000 the Historical Date Format would be mm/dd/yyyy. The use of a controlled vocabulary is strongly recommended to support interoperability.
    )
...
class ObjectAttributeSelector(Selector):
    ...
    refinedByOrderNumber: int | None = None  # Order number of the specific attribute.
    value: str | None = None  # Name of the attribute.
...
class PairedControlledVocabularyEntry(DdiCdiDataType):
    ...
    extent: Optional["ControlledVocabularyEntry"] = (
        None  # Describes the extent to which the parent term applies for the specific case using an external controlled vocabulary. When associated with a role from the CASRAI Contributor Roles Taxonomy an appropriate vocabulary should be specified as either "lead", "equal", or "supporting".
    )
...
class ProvenanceInformation(DdiCdiDataType):
    ...
    provenanceStatement: list["InternationalString"] = (
        None  # A statement of any changes in ownership and custody of the resource since its creation that are significant for its authenticity, integrity, and interpretation.
    )
...
class RationaleDefinition(DdiCdiDataType):
    ...
    rationaleCode: Optional["ControlledVocabularyEntry"] = (
        None  # Rationale code is primarily for internal processing flags within an organization or system. Supports the use of an external controlled vocabulary.
    )
    rationaleDescription: Optional["InternationalString"] = (
        None  # Textual description of the rationale/purpose for the version change to inform users as to the extent and implication of the version change. May be expressed in multiple languages.
    )
...
class Reference(DdiCdiDataType):
    ...
    ddiReference: Optional["InternationalRegistrationDataIdentifier"] = None  # A DDI type reference to a DDI object.
    deepLink: Optional["Selector"] = (
        None  # The selector refers to the object identifier by the ddiReference and has deep linking purposes.
    )
    description: str | None = None  # Human-readable description of the reference.
    location: Optional["InternationalString"] = (
        None  # The location of the referenced resource, as appropriate to support retrieval.
    )
    nonDdiReference: list["NonDdiIdentifier"] | None = (
        None  # A non-DDI reference to any object using a system of identification which is not supported by a URI.
    )
    semantic: Optional["ControlledVocabularyEntry"] = (
        None  # External qualifier to describe the purpose or meaning of the reference.
    )
    uri: str | None = None  # A URI to any object.
    validType: list[str] | None = (
        None  # The expected type of the reference (e.g., the class or element according to the schema of the referenced resource).
    )
...
class SpatialCoordinate(DdiCdiDataType):
    ...
    content: str | None = None  # The value of the coordinate expressed as a string.
    coordinate_type: Optional["PointFormat"] = (
        None  # Identifies the type of point coordinate system using a controlled vocabulary. Point formats include decimal degree, degrees minutes seconds, decimal minutes, meters, and feet.
    )
...
class SpatialPoint(DdiCdiDataType):
    ...
    xCoordinate: Optional["SpatialCoordinate"] = (
        None  # An X coordinate (latitudinal equivalent) value and format expressed using the Spatial Coordinate structure.
    )
    yCoordinate: Optional["SpatialCoordinate"] = (
        None  # A Y coordinate (longitudinal equivalent) value and format expressed using the Spatial Coordinate structure.
    )
...
class StructureSpecification(DdiCdiDataType):
    ...
    reflexive: "MemberRelationshipScope" = (
        None  # Members of the selected scope of the collection are related to themselves.
    )
...
class TimeRole(SpecializationRole):
    ...
    time: Optional["ControlledVocabularyEntry"] = (
        None  # Holds a value from an external controlled vocabulary defining the time role.
    )
...
class TypedString(DdiCdiDataType):
    ...
    typeOfContent: Optional["ControlledVocabularyEntry"] = (
        None  # Optional use of a controlled vocabulary to specifically type the associated content.
    )
...
class WebLink(DdiCdiDataType):
    ...
    typeOfWebsite: "ControlledVocabularyEntry" = (
        None  # The type of Website URL, for example personal, project, organization, division, etc.
    )
...
class XsdAnyUri(DdiCdiDataType):
    ...
    def add_to_rdf_graph(self, _g: Graph) -> URIRef:
...
class XsdDate(DdiCdiDataType):
    ...
    def add_to_rdf_graph(self, _g: Graph) -> Literal:
