...
def test_cli_stream_default(tmp_path):
    ...
    result = runner.invoke(app, ["ddil-stream", xml_path, "--output", str(out_file), "--limit", "5"])
...
def test_cli_stream_filter(tmp_path):
    ...
    result = runner.invoke(
        app, ["ddil-stream", xml_path, "--output", str(out_file), "--filter", "concept", "--limit", "10"]
    )
    assert result.exit_code == 0
...
def test_cli_stream_format_xml(tmp_path):
    ...
    result = runner.invoke(app, ["ddil-stream", xml_path, "--output", str(out_file), "--limit", "2", "--format", "xml"])
    assert result.exit_code == 0
    content = out_file.read_text(encoding="utf-8")
    assert "<" in content
    assert ">" in content
...
def test_cli_stream_stats(tmp_path):
    xml_path = sample_xml_path()
    out_file = tmp_path / "output.json"
    result = runner.invoke(app, ["ddil-stream", xml_path, "--output", str(out_file), "--limit", "2"])
    assert result.exit_code == 0
...
def test_cli_stream_no_stats(tmp_path):
    xml_path = sample_xml_path()
    out_file = tmp_path / "output.json"
    result = runner.invoke(app, ["ddil-stream", xml_path, "--output", str(out_file), "--limit", "2", "--no-stats"])
    assert result.exit_code == 0
...
def test_cli_stream_pretty(tmp_path):
    ...
    result = runner.invoke(app, ["ddil-stream", xml_path, "--output", str(out_file_json), "--limit", "1", "--pretty"])
    assert result.exit_code == 0
...
def test_cli_stream_default_output_filename(tmp_path):
    ...
    result = runner.invoke(app, ["ddil-stream", str(input_xml), "--limit", "1"])
    assert result.exit_code == 0
...
def test_cli_stream_unlimited_by_default(tmp_path):
    xml_path = sample_xml_path()
    out_file = tmp_path / "output.json"
    result = runner.invoke(app, ["ddil-stream", xml_path, "--output", str(out_file)])
    assert result.exit_code == 0
    content = out_file.read_text(encoding="utf-8").strip()
    lines = content.split("\n")
