...
def test_deserialize_multiple_objects():
    ...
    instances = deserializer.deserialize(
        graph, root_types=["http://ddialliance.org/Specification/DDI-CDI/1.0/RDF/InstanceVariable"]
    )
...
def test_round_trip_preserves_data():
    ...
    var = InstanceVariable(name=[ObjectName(name="PrimaryName"), ObjectName(name="AlternateName")])
    uri = f"http://example.org/{uuid.uuid4()}"
    irdi = InternationalRegistrationDataIdentifier(
        dataIdentifier=uri, registrationAuthorityIdentifier="http://example.org/authority", versionIdentifier="2.0.0"
    )
    identifier = Identifier(ddiIdentifier=irdi)
    var.identifier = identifier
