...
This document outlines how to resolve discrepancies between local development environments (using UV workspaces) and CI environments (using Git-based dependencies), as well as how to handle type-checking errors in dynamic models.
...
If you update the remote dependency (e.g., push a fix to `rdf-toolkit` main) but don't update the `ddi-toolkit/uv.lock`, the CI will run on stale code, causing Pyrefly or test failures that don't appear locally.
...
## 2. Pyrefly Troubleshooting in Dynamic Models

### Attribute Errors on Generated Models
In the **Assistant Framework**, resources are often passed as base types (like `CDIResource`). Since DDI-CDI is a large, generated model, not all attributes (like `identifier`) exist on every subclass. Pyrefly will flag direct access or direct assignment as an error on union types.

### Safe Attribute Access and Assignment
Always use `hasattr` or `getattr` / `setattr` when interacting with attributes that may not be present on all resource types:

```python
# Instead of:
resource.identifier = model.Identifier()

# Use setattr for safe assignment on union/base types:
if hasattr(resource, "identifier"):
    setattr(resource, "identifier", model.Identifier())
```

This ensures the code remains compatible with the full breadth of the DDI-CDI specification while satisfying the Pyrefly type checker.
