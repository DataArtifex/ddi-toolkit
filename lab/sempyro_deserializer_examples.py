...
def example_deserialize_from_file():
    ...
    instances = deserializer.deserialize(
        loaded_graph, root_types=["http://ddialliance.org/Specification/DDI-CDI/1.0/RDF/InstanceVariable"]
    )
...
def example_handle_multiple_formats():
    ...
    for fmt in formats:
        ...
        try:
            ...
            print(
                f"  Deserialized successfully: {deserialized.name[0].name if deserialized and deserialized.name else 'Failed'}"
            )
